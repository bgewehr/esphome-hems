...
#ifdef ESP_PLATFORM
#include <esp_log.h>
static const char* kShipConnTag = "ship";
#define SHIP_LOGW(fmt, ...) ESP_LOGW(kShipConnTag, fmt, ##__VA_ARGS__)
#else
#define SHIP_LOGW(fmt, ...) fprintf(stderr, "SHIP WARN: " fmt "\n", ##__VA_ARGS__)
#endif
...
EebusError ShipConnectionTryStart(ShipConnection* self, WebsocketCreatorObject* websocket_creator) {
  ...
  if (self->msg_queue == NULL) {
    ...
    SHIP_LOGW("TryStart: queue create failed for SKI=%s", self->remote_ski ? self->remote_ski : "?");
  ...
  if (self->websocket == NULL) {
    ...
    SHIP_LOGW("TryStart: websocket create failed for SKI=%s", self->remote_ski ? self->remote_ski : "?");
    return kEebusErrorCommunicationBegin;
  }

#ifndef GTEST
  self->thread = EebusThreadCreate(ShipConnectionLoop, self, 20 * 1024);
  if (self->thread == NULL) {
    SHIP_CONNECTION_DEBUG_PRINTF("%s(), start thread failed\n", __func__);
    SHIP_LOGW("TryStart: thread create failed for SKI=%s", self->remote_ski ? self->remote_ski : "?");
    return kEebusErrorThread;
  }
  SHIP_LOGW("TryStart: SHIP connection started for SKI=%s", self->remote_ski ? self->remote_ski : "?");
#endif
...
void ShipConnectionSetSmeState(ShipConnection* self, SmeState state) {
  if (self->sme_state != state) {
    ...
    const char* ski = SHIP_CONNECTION_GET_REMOTE_SKI(self);
    SHIP_LOGW("state %d/%s (ski=%s)", (int)state, SmeStateToString(state), ski ? ski : "?");
    INFO_PROVIDER_HANDLE_SHIP_STATE_UPDATE(self->info_provider, ski, state, "");
  }
}
...
void ShipConnectionCloseWithError(ShipConnection* self, const char* err) {
  ...
  SHIP_LOGW("CloseWithError: %s (ski=%s)", err, self->remote_ski ? self->remote_ski : "?");
...
void SmeHandshakeAccessMethodsInit(ShipConnection* self) {
  SHIP_LOGW("AccessMethods: sending request (ski=%s)", self->remote_ski ? self->remote_ski : "?");
  ...
  if (error != kEebusErrorOk) {
    ...
    SHIP_LOGW("AccessMethods: request sent ok (ski=%s)", self->remote_ski ? self->remote_ski : "?");
...
EebusError SmeHandshakeAccessMethodsHandle(ShipConnection* self, ShipMessageDeserializeObject* deserialize) {
  ...
  if (!SmeHandshakeAccessMethodsCheckMessageVal(self, deserialize)) {
    SHIP_LOGW("AccessMethods: response validation FAILED (ski=%s)", self->remote_ski ? self->remote_ski : "?");
    return kEebusErrorCommunication;
  }
...
EebusError DataExchangeHandleReceive(ShipConnection* self, const MessageBuffer* buf) {
  ...
  if (deserialize == NULL) {
    SHIP_LOGW("DataExchange: deserialize NULL (ski=%s)", self->remote_ski ? self->remote_ski : "?");
    return kEebusErrorParse;
  }

  const MsgValueType message_type = SHIP_MESSAGE_DESERIALIZE_GET_VALUE_TYPE(deserialize);
  SHIP_LOGW("DataExchange recv type=%d (ski=%s)", (int)message_type, self->remote_ski ? self->remote_ski : "?");
  ...
  if (message_type == kData) {
    ...
    if (data != NULL) {
      SHIP_LOGW("DataExchange: SPINE data recv len=%u", (unsigned)(data->payload.data_size));
      ...
      SHIP_LOGW("DataExchange: kData but payload NULL (ski=%s)", self->remote_ski ? self->remote_ski : "?");
      ret = kEebusErrorParse;
    }

  } else if (message_type == kSmeConnectionAccessMethodsRequest) {
    SHIP_LOGW("DataExchange: received AccessMethodsRequest from remote (ski=%s)", self->remote_ski ? self->remote_ski : "?");
    ret = SmeHandshakeAccessMethodsRequestHandle(self);
  } else if (message_type == kSmeConnectionAccessMethods) {
    SHIP_LOGW("DataExchange: received AccessMethods response (ski=%s)", self->remote_ski ? self->remote_ski : "?");
...
EebusError DataExchangeHandleSendSpineData(ShipConnection* self, const MessageBuffer* buf) {
  SHIP_LOGW("DataExchange: SPINE send len=%u (ski=%s)", (unsigned)(buf->data_size), self->remote_ski ? self->remote_ski : "?");
