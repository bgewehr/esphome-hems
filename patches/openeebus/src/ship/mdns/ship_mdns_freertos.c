...
void MdnsProcessSearchResult(Mdns* mdns, mdns_search_once_t* search) {
  ...
  if (results == NULL) {
    ESP_LOGD("eebus_mdns", "mDNS PTR query: no _ship._tcp results");
  ...
  ESP_LOGI("eebus_mdns", "mDNS PTR query: %d _ship._tcp result(s)", result_count);
  ...
  while (r) {
    ...
    if (entry == NULL) {
      ...
      if (MdnsEntryIsValid(entry) && (strcmp(entry->ski, mdns->ski) != 0)) {
        ESP_LOGI("eebus_mdns", "  Found: %s ski=%s", entry->name ? entry->name : "?", entry->ski ? entry->ski : "?");
        MDNS_DEBUG_PRINTF("Added entry: %s, ski: %s\n", entry->name, entry->ski);
        VectorPushBack(mdns->found_entries, entry);
      } else {
        ESP_LOGI("eebus_mdns", "  Ignored: %s ski=%s valid=%d own=%d",
                 entry->name ? entry->name : "?", entry->ski ? entry->ski : "?",
                 MdnsEntryIsValid(entry),
                 entry->ski && (strcmp(entry->ski, mdns->ski) == 0));
...
void* MdnsBrowserLoop(void* parameters) {
  ...
  while (!mdns->cancel) {
    ...
    ESP_LOGD("eebus_mdns", "Sending mDNS PTR query for _ship._tcp");
