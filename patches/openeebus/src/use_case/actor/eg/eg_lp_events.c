...
/* Subscribe to ElectricalConnection (scenario 4) — eebus-go does this at connection time.
 * K40RF may require this subscription to show "EEBus OK" status. */
void OnEntityAddedHandleElectricalConnection(const EgLpUseCase* self, EntityRemoteObject* entity) {
  const UseCase* const use_case = USE_CASE(self);

  ElectricalConnectionClient el_connection;
  const EebusError err = ElectricalConnectionClientConstruct(&el_connection, use_case->local_entity, entity);
  if (err != kEebusErrorOk) {
    return;
  }

  FeatureInfoClient* feature_info = &el_connection.feature_info_client;
  if (!HasSubscription(feature_info)) {
    Subscribe(feature_info);
  }

  ElectricalConnectionClientRequestDescriptions(&el_connection, NULL, NULL);
  ElectricalConnectionClientRequestParameterDescriptions(&el_connection, NULL, NULL);
}
