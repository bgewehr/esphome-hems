...
EebusError RegisterService(ShipMdnsObject* self) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE("eebus", "mDNS: mdns_service_add() failed: %d", err);
    return kEebusErrorInit;
  }

  ESP_LOGW("eebus", "mDNS: registered _ship._tcp \"%s\" port=%d register=%s ski=%.8s...",
           mdns->service_name, mdns->port, register_str, mdns->ski ? mdns->ski : "?");
...
EebusError Start(ShipMdnsObject* self) {
  ...
  ESP_LOGW("eebus", "mDNS: hostname=%s.local", host_name);
...
void SetAutoaccept(ShipMdnsObject* self, bool autoaccept) {
  ...
  ESP_LOGW("eebus", "mDNS: register TXT -> %s", val);
