...
static void spine_event_handler(const EventPayload* payload, void* ctx) {
  ...
  {
    const std::string& paired = self->remote_ski();
    if (!paired.empty()) {
      if (paired != ski) return;  /* event from a device we are not paired with */
    } else if (!self->is_pairing_mode()) {
      return;  /* no paired device, pairing not active — ignore all events */
    }
  }
...
static bool SR_IsWaitingForTrustAllowed(const ServiceReaderObject* o, const char* ski) {
  ...
  if (!(self->pairing_mode_active_ && (int32_t)(millis() - self->pairing_deadline_ms_) < 0)) return false;
  /* Never auto-trust a device that is already paired with ANOTHER EG instance —
   * the WP gateway probes every EG port and would otherwise win the window. */
  if (esphome::eebus_eg::EebusEgComponent::ski_paired_elsewhere(self, ski)) {
    ESP_LOGW("eebus_eg", "%s: pairing candidate %s belongs to another instance — rejected",
...
std::vector<EebusEgComponent*> EebusEgComponent::instances_;

bool EebusEgComponent::ski_paired_elsewhere(const EebusEgComponent* self, const char* ski) {
  for (auto* inst : instances_) {
    if (inst == self || inst == nullptr) continue;
    if (!inst->remote_ski_.empty() && inst->remote_ski_ == ski) return true;
  }
  return false;
}
...
void EebusEgComponent::setup() {
  ...
  if (!load_cert_nvs_(&cert, &cl, &key, &kl)) {
    /* No cert in NVS: generate a fresh one.
     * IMPORTANT: cert generation is kept in this branch so the heavy
     * mbedtls stack (key_buf[2048] + cert_buf[4096] + entropy/drbg contexts)
     * does NOT coexist with the sha1 duplicate-check locals below. */
    ESP_LOGI(TAG, "%s: no cert — generating...", instance_name_.c_str());
    if (!load_or_generate_cert_(&cert, &cl, &key, &kl)) {
      ESP_LOGE(TAG, "%s: cert generation failed", instance_name_.c_str());
      mark_failed(); return;
    }
  } else {
    /* Cert found in NVS.  Detect duplicate: two EG instances may have
     * inherited the same cert (both migrated from a shared legacy namespace).
     * Compare SHA-1 of raw DER bytes using a heap-allocated context so the
     * sha1 state does not inflate setup()'s stack frame. */
    uint8_t cert_sha1[20] = {};
    mbedtls_sha1_context* sha1 = (mbedtls_sha1_context*)malloc(sizeof(mbedtls_sha1_context));
    if (sha1) {
  ...
  if (!start_eebus_service_(cert, cl, key, kl)) {
    /* Self-healing: cert may be invalid (e.g. old firmware generated cert without
     * proper SKI extension).  Erase and regenerate once. */
  ...
  free(cert); free(key);
  startup_mdns_at_ms_ = millis() + 15000;
...
void EebusEgComponent::on_entity_connect(const EntityAddressType* addr) {
  if (remote_ski_.empty()) {
    /* SKI-first rule: a legitimate pairing always completes SHIP DataExchange
     * (which sets remote_ski_) before any SPINE entity appears. An entity
     * connect without a paired SKI is a foreign or premature session — never
     * adopt it as our device. */
    ESP_LOGD(TAG, "%s: entity connect without paired SKI ignored", instance_name_.c_str());
    return;
  }
    ...
    uint8_t** key_out,  size_t* kl_out)
{
  ...
  do {
    ...
    *key_out  = (uint8_t*)malloc((size_t)key_len);
    if (!*cert_out || !*key_out) { free(*cert_out); free(*key_out); *cert_out = *key_out = nullptr; break; }
    memcpy(*cert_out, cert_p, (size_t)cert_len); *cl_out = (size_t)cert_len;
    memcpy(*key_out,  key_p,  (size_t)key_len);  *kl_out = (size_t)key_len;

    /* Persist to NVS — best-effort; service starts from memory even if this fails */
    store_cert_nvs_(cert_p, (size_t)cert_len, key_p, (size_t)key_len);
